...
* **智能例行活動分析：** 高階背景引擎，24/7 監控用戶行為以識別日常生活模式。請參見下方的 [深度解析](#-例行活動分析深度解析) 章節。

---

## 🔍 例行活動分析深度解析

**例行活動分析 (Routine Activity Analysis)** 引擎超越了單純的經緯度記錄，為用戶提供了在時間與空間維度下時間分配的全面崩解圖。

### 🧠 智能狀態機
分析的核心是一個強大的狀態機，將時間歸類為四種主要狀態：
- **🏠 居家 (HOME)：** 當在設定的住家坐標自定義半徑內停留在原地時，系統會自動偵測。
- **💼 辦公 (WORK)：** 當停留在預先定義的工作場所時識別。
- **🏃 移動 (MOVING)：** 由高置信度的活動感測（步行、跑步、騎行或開車）觸發。
- **🌲 戶外停留 (OUTDOOR STAY)：** 在主要區域（如公園、商店或朋友家）之外偵測到的靜止期間。

### 🛠 技術實現細節
- **活動感測 API (Activity Recognition API)：** 整合 Google Play 服務進行低功耗行為感測，與連續 GPS 輪詢相比，可顯著降低電池消耗。
- **時間去抖動 (Temporal Debouncing)：** 實現了 5 分鐘的「靜止去抖動」邏輯。引擎要求在從 `MOVING` 切換到靜止狀態之前，必須有一段持續的靜止期，這能有效過濾掉等紅綠燈或短暫停頓造成的傳感器雜訊。
- **坐標匹配：** 使用高精度距離算法，將當前坐標與用戶定義的設定文件進行匹配，允許精細的地理圍欄控制（預設 100 公尺半徑）。
- **架構設計：** 採用解耦的服務架構（`RoutineAnalysisManager` 與 `ActivityUpdateReceiver`），確保即使主應用程式被內存回收，分析仍能持續運作。

### 📊 數據洞察與匯出
- **自定義圓餅圖：** 使用 Jetpack Compose `Canvas` 自定義繪製的高性能視覺化組件，提供流暢的每日總結。
- **科研級 JSON 匯出：** 所有分析轉移記錄均可匯出為結構化的 JSON 數據，方便進行空間研究或個人生產力審計等進階外部分析。
